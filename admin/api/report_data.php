...
try {
    ...
    $stmt = $pdo->prepare("SELECT helpful_staff_names FROM feedbacks $dateFilter");
    $stmt->execute($params);
    $staffCounts = [];
    while ($row = $stmt->fetch()) {
            ...
            foreach ($parts as $part) {
                ...
                if ($part !== "") {
                    ...
                    if (!isset($staffCounts[$key])) {
                        $staffCounts[$key] = ["name" => ucwords($part), "count" => 0];
                    }
    ...
    usort($staffCounts, function ($a, $b) { return $b["count"] <=> $a["count"]; });
    ...
    $response = [
        ...
        "front_of_house" => [
            ...
            ["label" => "Telephone Operator", "avg" => toTenScale($summary["avg_telephone"] ?? 0)],
