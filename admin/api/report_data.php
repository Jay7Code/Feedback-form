...
try {
    ...
    $stmt = $pdo->prepare(
        "SELECT guest_name, room_no, overall_rating, frontdesk_comments, fnb_comments, suggestions_future, other_comments, created_at FROM feedbacks $dateFilter ORDER BY created_at DESC LIMIT 20",
    );
    $stmt->execute($params);
